...
# Internal predicate: a leading-zero-stripped copy of ndar:src_subject_id,
# materialized so cross-file subject joins are indexed term matches rather than
# per-row REPLACE() filters.  MUST match the value used in
# nidm.experiment.tools.nidm_queryai so a query generated there runs verbatim
# through `pynidm query` too.
...
def _make_query_graph():
    """Return an Oxigraph-backed graph when the optional ``oxrdflib`` package is
    installed (Rust SPARQL engine -- far faster on large/merged NIDM graphs),
    else the default in-memory rdflib store.  Override with
    ``PYNIDM_QUERY_ENGINE`` (``auto`` (default) | ``oxigraph`` | ``rdflib``)."""
    requested = environ.get("PYNIDM_QUERY_ENGINE", "auto").strip().lower()
...
def _materialize_normalized_subject_ids(graph):
    """Add ``(person, _NORM_SID_URI, zero-stripped id)`` for every
    ``ndar:src_subject_id`` in *graph* so subjects can be joined on an indexed
    term across files (incl. zero-padding differences).  Idempotent."""
    norm = URIRef(_NORM_SID_URI)
    sid = URIRef(_NDAR_SID_URI)
    for person, raw in list(graph.subject_objects(sid)):
        graph.add((person, norm, Literal(re.sub(r"^0+(?=.)", "", str(raw)))))
...
def sparql_query_nidm(nidm_file_list, query, output_file=None, return_graph=False):
    ...
    rdf_graph_parse = _make_query_graph()
    for nidm_file in nidm_file_list:
        rdf_graph_parse += OpenGraph(nidm_file)
    # only materialize the normalized-subject-id index when the query actually
    # references it (e.g. a query generated by `pynidm queryai`), so routine
    # internal query helpers are untouched
    if _NORM_SID_URI in query:
        _materialize_normalized_subject_ids(rdf_graph_parse)
